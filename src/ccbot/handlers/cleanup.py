...
async def clear_topic_state(
    ...
    Cleans up:
      ...
      - the topic's message queue and its worker task
    ...
    # Stop the topic's queue worker and discard anything still queued for it
    await shutdown_topic_queue(user_id, thread_id)
